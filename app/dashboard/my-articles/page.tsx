...
export default function MyArticles() {
    ...
    useEffect(() => {
        ...
        if (userData) {
            try {
                ...
                console.log("DEBUG: user object from localStorage:", user);

                // Priorizamos el nombre del rol como string, ya que los IDs pueden variar entre entornos
                const r_name = (
                    user.role ||
                    user.role_name ||
                ...
                if (r_name === 'student') {
                    ...
                    // Fallback a IDs solo si el nombre no está claro
    ...
    const fetchArticles = async (page: number) => {
        setLoading(true)
        try {
            const response = await getMyArticles(page, limit)
