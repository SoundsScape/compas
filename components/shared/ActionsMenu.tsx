    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button disabled={!canEditOrDelete} variant="ghost" size="sm" className="h-8 w-8 p-0 hover:bg-accent/90 hover:text-accent-foreground text-white cursor-pointer">
                ...
                {canValidate && (
                    <>
                        {article.status === 'Pendiente' ? (
                            ...
                            >
                                <CheckCircle className="size-4 text-green-500 group-hover:text-green-700" />
                                Validar
                            </DropdownMenuItem>
                        ) : (
                            <DropdownMenuItem
                ...
                {canEditOrDelete && (
                    <>
                        <DropdownMenuItem asChild>
                            <Link href={`/articles/edit/${article.id}`} className="cursor-pointer gap-2 flex items-center group">
                                <Pencil className="size-4 text-accent group-hover:text-yellow-600" />
                                Editar
                            </Link>
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
